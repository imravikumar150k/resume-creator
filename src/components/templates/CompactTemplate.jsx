...
const CompactTemplate = forwardRef(function CompactTemplate({ data }, ref) {
  ...
  return (
    <div ref={ref} className="resume-preview bg-white p-8 print:p-0 max-w-[8.5in] mx-auto font-sans text-xs leading-snug">
      ...
      {experience.length > 0 && experience.some((e) => e.title || e.company) && (
        <section className="mb-2">
          <h2 className="text-[10px] font-bold uppercase text-gray-500 mb-1">Experience</h2>
      ...
      {education.length > 0 && education.some((e) => e.degree || e.institution) && (
        <section className="mb-2">
          <h2 className="text-[10px] font-bold uppercase text-gray-500 mb-1">Education</h2>
          {education.map((entry) => (
      ...
      {skills.length > 0 && (
        <section className="mb-2">
          <h2 className="text-[10px] font-bold uppercase text-gray-500 mb-1">Skills</h2>
      ...
      {projects.length > 0 && projects.some((p) => p.name) && (
        <section className="mb-2">
          <h2 className="text-[10px] font-bold uppercase text-gray-500 mb-1">Projects</h2>
          {projects.map((entry) => (
            entry.name && (
              <div key={entry.id} className="mb-1">
                <span className="font-bold text-gray-900">{entry.name}</span>
                {entry.link && <span className="text-[10px] text-blue-600 ml-1">{entry.link}</span>}
