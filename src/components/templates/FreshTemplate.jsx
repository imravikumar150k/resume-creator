...
const FreshTemplate = forwardRef(function FreshTemplate({ data }, ref) {
  ...
  return (
    <div ref={ref} className="resume-preview bg-white p-8 print:p-0 max-w-[8.5in] mx-auto font-sans text-sm leading-relaxed">
      <header className="mb-5 bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 print:rounded-none">
        <h1 className="text-2xl font-bold text-emerald-800">{personalInfo.name || 'Your Name'}</h1>
        <div className="text-emerald-600 text-xs mt-1 flex flex-wrap gap-x-3">
      ...
      {summary && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-emerald-700 mb-1">About Me</h2>
      ...
      {experience.length > 0 && experience.some((e) => e.title || e.company) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-emerald-700 mb-2">Experience</h2>
      ...
      {education.length > 0 && education.some((e) => e.degree || e.institution) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-emerald-700 mb-2">Education</h2>
          {education.map((entry) => (
      ...
      {skills.length > 0 && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-emerald-700 mb-2">Skills</h2>
          <div className="flex flex-wrap gap-1.5">
            {skills.map((skill, i) => (
              <span key={i} className="bg-emerald-50 text-emerald-700 text-xs px-2 py-0.5 rounded-full border border-emerald-200">{skill}</span>
            ))}
          </div>
        </section>
      )}
      ...
      {projects.length > 0 && projects.some((p) => p.name) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-emerald-700 mb-2">Projects</h2>
          {projects.map((entry) => (
            entry.name && (
              <div key={entry.id} className="mb-2">
                <div className="font-semibold text-gray-900">{entry.name}
                  {entry.link && <span className="text-xs text-emerald-600 ml-2">{entry.link}</span>}
                </div>
