...
const BoldTemplate = forwardRef(function BoldTemplate({ data, accentColor = '#111827' }, ref) {
  const { personalInfo, summary, experience, education, skills, projects } = data

  return (
    <div ref={ref} className="resume-preview bg-white max-w-[8.5in] mx-auto font-sans text-sm leading-relaxed">
      <header className="text-white px-8 py-6" style={{ backgroundColor: accentColor }}>
      ...
      <div className="px-8 pb-8 pt-6">
        ...
        {experience.length > 0 && experience.some((e) => e.title || e.company) && (
          <section className="mb-5">
            <h2 className="text-sm font-bold uppercase mb-1" style={{ color: accentColor }}>Experience</h2>
        ...
        {education.length > 0 && education.some((e) => e.degree || e.institution) && (
          <section className="mb-5">
            <h2 className="text-sm font-bold uppercase mb-1" style={{ color: accentColor }}>Education</h2>
            {education.map((entry) => (
        ...
        {skills.length > 0 && (
          <section className="mb-5">
            <h2 className="text-sm font-bold uppercase mb-1" style={{ color: accentColor }}>Skills</h2>
        ...
        {projects.length > 0 && projects.some((p) => p.name) && (
          <section className="mb-5">
            <h2 className="text-sm font-bold uppercase mb-1" style={{ color: accentColor }}>Projects</h2>
            {projects.map((entry) => (
              entry.name && (
                <div key={entry.id} className="mb-2">
                  <div className="font-bold text-gray-900">
                    {entry.name}
                    {entry.link && <a href={entry.link.startsWith('http') ? entry.link : `https://${entry.link}`} target="_blank" rel="noopener noreferrer" className="text-xs ml-2 font-normal" style={{ color: accentColor }}>{entry.link}</a>}
                  </div>
