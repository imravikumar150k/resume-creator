...
const GeometricTemplate = forwardRef(function GeometricTemplate({ data }, ref) {
  ...
  return (
    <div ref={ref} className="resume-preview bg-white p-8 print:p-0 max-w-[8.5in] mx-auto font-sans text-sm leading-relaxed">
      <header className="mb-5 relative">
        <div className="absolute top-0 left-0 w-16 h-16 bg-orange-500 opacity-20 rotate-45 -translate-x-4 -translate-y-4"></div>
        ...
        <div className="h-1 bg-gradient-to-r from-orange-500 to-red-500 mt-3"></div>
      </header>

      {summary && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-orange-600 mb-1">Summary</h2>
      ...
      {experience.length > 0 && experience.some((e) => e.title || e.company) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-orange-600 mb-2">Experience</h2>
          {experience.map((entry) => (
            (entry.title || entry.company) && (
              <div key={entry.id} className="mb-3 pl-3 border-l-4 border-orange-200">
      ...
      {education.length > 0 && education.some((e) => e.degree || e.institution) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-orange-600 mb-2">Education</h2>
          {education.map((entry) => (
            (entry.degree || entry.institution) && (
              <div key={entry.id} className="mb-2 pl-3 border-l-4 border-orange-200">
                <div className="flex justify-between items-baseline">
      ...
      {skills.length > 0 && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-orange-600 mb-2">Skills</h2>
          <div className="flex flex-wrap gap-1.5">
            {skills.map((skill, i) => (
              <span key={i} className="bg-orange-50 text-orange-800 text-xs px-2 py-0.5 border border-orange-200">{skill}</span>
            ))}
          </div>
        </section>
      )}
      ...
      {projects.length > 0 && projects.some((p) => p.name) && (
        <section className="mb-4">
          <h2 className="text-sm font-bold text-orange-600 mb-2">Projects</h2>
          {projects.map((entry) => (
            entry.name && (
              <div key={entry.id} className="mb-2 pl-3 border-l-4 border-orange-200">
                <div className="font-semibold text-gray-900">{entry.name}
                  {entry.link && <span className="text-xs text-orange-600 ml-2">{entry.link}</span>}
                </div>
